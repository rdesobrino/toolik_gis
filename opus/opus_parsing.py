    ...
    for file in os.listdir(emls): ##TODO add CRS?
        if not "aborting" in file: # only search successful opuses
            with open(os.path.join(emls, file), "r") as eml:
                ...
                long = [char for char in long_search if char != ''][:3]
                long_dd = str(-1 * (float(long[0]) + float(long[1]) / 60 + float(long[2]) / 3600))

                el_search = text[text.find("EL HGT:") + len("EL HGT:"):].split(" ")
                ell = [char[:-3] for char in el_search if char != ''][0].split("(")[0]
                ...
                row = [date, name, easting, northing, ortho, cors[:-1], ant_type, ant_height, rms, dur, "", lat_dd, long_dd, ell]
